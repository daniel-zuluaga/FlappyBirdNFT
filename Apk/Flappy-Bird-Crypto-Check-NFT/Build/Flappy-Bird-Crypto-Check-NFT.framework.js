...
function _EcRecoverJS(message, signature) {
 window.web3gl.ecRecover(UTF8ToString(message), UTF8ToString(signature));
}

function _EcRecoverResponse() {
...
function _SendAsyncJs(method, parameters) {
 window.web3gl.sendAsync(UTF8ToString(method), UTF8ToString(parameters));
}

function _SendAsyncResponse() {
...
function _SendContractJs(method, abi, contract, args, value, gasLimit, gasPrice) {
 window.web3gl.sendContract(UTF8ToString(method), UTF8ToString(abi), UTF8ToString(contract), UTF8ToString(args), UTF8ToString(value), UTF8ToString(gasLimit), UTF8ToString(gasPrice));
}
...
function _SendTransactionJs(to, value, gasLimit, gasPrice) {
 window.web3gl.sendTransaction(UTF8ToString(to), UTF8ToString(value), UTF8ToString(gasLimit), UTF8ToString(gasPrice));
}

function _SendTransactionJsData(to, value, gasLimit, gasPrice, data) {
 window.web3gl.sendTransactionData(UTF8ToString(to), UTF8ToString(value), UTF8ToString(gasLimit), UTF8ToString(gasPrice), UTF8ToString(data));
}
...
var asmLibraryArg = {
 "ClearResponseJs": _ClearResponseJs,
 "ConnectAccount": _ConnectAccount,
 "EcRecoverJS": _EcRecoverJS,
 "EcRecoverResponse": _EcRecoverResponse,
 ...
 "SendAsyncError": _SendAsyncError,
 "SendAsyncJs": _SendAsyncJs,
 "SendAsyncResponse": _SendAsyncResponse,
